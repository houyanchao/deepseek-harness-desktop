...
import { installDsh, parsePnpmPercent, resolveVersion } from './dsh-install.mjs'
import {
  installPlugin,
...
import { curatedPlugins, fetchUpdateManifest, recommendedDshVersion } from './update-manifest.mjs'
 ...
 * @param {(percent: number | null) => void} [onProgress] splash progress callback.
 * @returns {Promise<string>} the dsh entry path.
 */
export async function ensureDsh(onStatus, onProgress) {
  const existing = dshEntry()
  if (existing !== null) return existing
  onStatus('正在获取 DeepSeek Harness 版本…')
  // The manifest (when configured and reachable) recommends a version — the
  // first entry of dsh.versions; otherwise the registry dist-tag decides.
  // First boot must not fail just because the manifest host is down while the
  // registry is fine.
  let version = null
  if (UPDATE_MANIFEST_URL !== null) {
    try {
      version = recommendedDshVersion(await fetchUpdateManifest(UPDATE_MANIFEST_URL))
    } catch {
      version = null
    }
  }
 ...
 * @param {(percent: number | null) => void} [onProgress] splash progress callback.
 */
export async function preinstallPlugins(onStatus, onProgress) {
  ...
  try {
    for (const [index, plugin] of missing.entries()) {
      ...
      // Each plugin is its own pnpm run, so the bar restarts per plugin
      // rather than pretending to track the whole set.
      onProgress?.(null)
      log.write(`\n--- ${plugin.spec} ---\n`)
